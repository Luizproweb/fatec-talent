...
class CursoList extends TPage
{
    ...
    private static $formName = 'form_CursoList';
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->setFormTitle("Listagem de listagem de cursos");
        $this->limit = 20;
        ...
        if(TSession::getValue('CursoList_expand_start_hidden') === false)
        {
            $startHidden = false;
        }
        elseif(TSession::getValue('CursoList_expand_start_hidden') === true)
        {
            $startHidden = true; 
        }
        ...
        $btn_onshow = $this->form->addAction("Cadastrar", new TAction(['CursoForm', 'onShow']), 'fas:plus #69aa46');
        ...
        $action_onTurnOnOff = new TDataGridAction(array('CursoList', 'onTurnOnOff'));
        ...
        $button_atualizar->setAction(new TAction(['CursoList', 'onRefresh']), "Atualizar");
        ...
        $button_limpar_filtros->setAction(new TAction(['CursoList', 'onClearFilters']), "Limpar filtros");
        ...
        $button_cadastrar->setAction(new TAction(['CursoForm', 'onShow']), "Cadastrar");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['CursoList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['CursoList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['CursoList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['CursoList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Cursos","Listagem de Cursos"]));
        }
    ...
    public static function onExpandForm($param = null)
    {
        try
        {
            ...
            if(TSession::getValue('CursoList_expand_start_hidden') === false)
            {
                TSession::setValue('CursoList_expand_start_hidden', true);
            }
            elseif(TSession::getValue('CursoList_expand_start_hidden') === true)
            {
                TSession::setValue('CursoList_expand_start_hidden', false);
            }
            else
            {
                TSession::setValue('CursoList_expand_start_hidden', !$startHidden);
            }
