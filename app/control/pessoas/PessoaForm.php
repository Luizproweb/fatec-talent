...
class PessoaForm extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $row1 = $cadastro_pessoa->addFields([new TLabel("Id:", null, '14px', null, '100%'),$id],[new TLabel("Usuário do Sistema:", '#000000', '14px', null, '100%'),$system_user]);
        ...
        $row3 = $cadastro_pessoa->addFields([new TLabel("Site :", null, '14px', null, '100%'),$site],[new TLabel("Currículo:", null, '14px', null),$anexo_curriculo]);
        $row3->layout = ['col-sm-6','col-sm-6'];

        $row4 = $cadastro_pessoa->addFields([new TLabel("E-mail:", null, '14px', null, '100%'),$email],[new TLabel("Ativo :", '#ff0000', '14px', null, '100%'),$ativo]);
        $row4->layout = ['col-sm-4','col-sm-3'];

        $row5 = $cadastro_pessoa->addFields([new TLabel("Observação :", null, '14px', null, '100%'),$observacao]);
        ...
        $row6 = $this->detailFormEnderecoPessoa->addFields([new TLabel("CEP :", null, '14px', null, '100%'),$endereco_pessoa_cep]);
        $row6->layout = [' col-sm-3'];

        $row7 = $this->detailFormEnderecoPessoa->addFields([new TLabel("Endereço :", null, '14px', null, '100%'),$endereco_pessoa_endereco,$endereco_pessoa_id],[new TLabel("Número :", null, '14px', null, '100%'),$endereco_pessoa_numero],[new TLabel("Bairro :", null, '14px', null, '100%'),$endereco_pessoa_bairro]);
        $row7->layout = [' col-sm-6',' col-sm-2',' col-sm-4'];

        $row8 = $this->detailFormEnderecoPessoa->addFields([new TLabel("Complemento :", null, '14px', null, '100%'),$endereco_pessoa_complemento],[new TLabel("Cidade:", '#ff0000', '14px', null, '100%'),$endereco_pessoa_cidade_id]);
        $row8->layout = ['col-sm-6',' col-sm-6'];

        $row9 = $this->detailFormEnderecoPessoa->addFields([new TLabel("Tipo de Endereço:", '#ff0000', '14px', null, '100%'),$endereco_pessoa_tipo_endereco_id]);
        $row9->layout = ['col-sm-6'];

        $row10 = $this->detailFormEnderecoPessoa->addFields([$button_adicionar_endereco_pessoa]);
        $row10->layout = [' col-sm-12'];

        $row11 = $this->detailFormEnderecoPessoa->addFields([new THidden('endereco_pessoa__row__id')]);
        ...
        $action_onEditDetailEndereco = new TDataGridAction(array('PessoaForm', 'onEditDetailEndereco'));
        ...
        $action_onDeleteDetailEndereco = new TDataGridAction(array('PessoaForm', 'onDeleteDetailEndereco'));
        ...
        $row12 = $cadastro_pessoa->addFields([$this->detailFormEnderecoPessoa]);
        $row12->layout = [' col-sm-12'];

        $row13 = $this->form->addFields([$cadastro_pessoa]);
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Pessoas","Cadastro de Usuários"]));
        }
