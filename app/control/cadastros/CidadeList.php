...
class CidadeList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $estado_id = new TDBCombo('estado_id', 'u316696823_fatec_talent', 'Estado', 'id', '{id}','id asc'  );
        ...
        $row1 = $this->form->addFields([new TLabel("Nome :", null, '14px', null, '100%'),$nome],[new TLabel("Código IBGE :", null, '14px', null, '100%'),$codigo_ibge]);
        $row1->layout = ['col-sm-6',' col-sm-6'];

        $row2 = $this->form->addFields([new TLabel("Estado:", null, '14px', null, '100%'),$estado_id],[new TLabel("Ativo:", null, '14px', null, '100%'),$ativo]);
        ...
        if(TSession::getValue('CidadeList_expand_start_hidden') === false)
        {
            $startHidden = false;
        }
        elseif(TSession::getValue('CidadeList_expand_start_hidden') === true)
        {
            $startHidden = true; 
        }
        ...
        $column_estado_sigla = new TDataGridColumn('{estado->sigla}', "Estado", 'left');
        ...
        $this->datagrid->addColumn($column_estado_sigla);
        ...
        $action_onTurnOnOff = new TDataGridAction(array('CidadeList', 'onTurnOnOff'));
    ...
    public static function onExpandForm($param = null)
    {
        try
        {
            ...
            if(TSession::getValue('CidadeList_expand_start_hidden') === false)
            {
                TSession::setValue('CidadeList_expand_start_hidden', true);
            }
            elseif(TSession::getValue('CidadeList_expand_start_hidden') === true)
            {
                TSession::setValue('CidadeList_expand_start_hidden', false);
            }
            else
            {
                TSession::setValue('CidadeList_expand_start_hidden', !$startHidden);
            }
