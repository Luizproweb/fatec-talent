...
class PaisList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->setFormTitle("Listagem de pais");
        $this->limit = 20;
        ...
        $row1 = $this->form->addFields([new TLabel("Nome :", null, '14px', null, '100%'),$nome],[new TLabel("Sigla:", null, '14px', null, '100%'),$sigla],[new TLabel("Ativo :", null, '14px', null, '100%'),$ativo]);
        ...
        if(TSession::getValue('PaisList_expand_start_hidden') === false)
        {
            $startHidden = false;
        }
        elseif(TSession::getValue('PaisList_expand_start_hidden') === true)
        {
            $startHidden = true; 
        }
        ...
        $action_onTurnOnOff = new TDataGridAction(array('PaisList', 'onTurnOnOff'));
        ...
        $button_cadastrar->setAction(new TAction(['PaisForm', 'onShow']), "Cadastrar");
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Cadastros","Pais"]));
        }
    ...
    public static function onExpandForm($param = null)
    {
        try
        {
            ...
            if(TSession::getValue('PaisList_expand_start_hidden') === false)
            {
                TSession::setValue('PaisList_expand_start_hidden', true);
            }
            elseif(TSession::getValue('PaisList_expand_start_hidden') === true)
            {
                TSession::setValue('PaisList_expand_start_hidden', false);
            }
            else
            {
                TSession::setValue('PaisList_expand_start_hidden', !$startHidden);
            }
